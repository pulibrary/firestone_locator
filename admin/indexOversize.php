...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "InsertCallNum")) {
	if (isset($_POST["id_cn"])&&$_POST["id_cn"]!="") {
		$selectSQL = sprintf("SELECT * FROM lctr_Oversize_cn WHERE id_cn=%s",
		...
		$x_point_cn = 0;
		$y_point_cn = 0;
		$selectSQL = "SELECT * FROM lctr_Oversize_cn ORDER BY date_cn DESC LIMIT 0,1";
	}
	
	
	$insertSQL = sprintf("REPLACE INTO lctr_Oversize_cn (id_cn, left_cn, right_cn, LocationDisplayName_cn,  BuildingCode_cn, FloorDB_cn, LocationMap_cn, Image_cn, message_cn, x_point_cn, y_point_cn, date_cn) VALUES (%d, %s, %s, %s, %s, %s, %s, %s, %s, $x_point_cn, $y_point_cn, NOW())",
			GetSQLValueString($_POST['id_cn'], "int"),
	...
	if ($dbconnects["stage"]->query($insertSQL) ) {
		...
		$subRecord = $subResult->fetch_assoc();
			...
			$success .= "<strong>Range updated:</strong><br/>\n";
...
if (isset($_GET['order'])&&$_GET["order"]!="") {
	$order = $_GET['order'];
	$query_rsAdminCallNum = "SELECT * FROM lctr_Oversize_cn ORDER BY $order ASC";
} else {
	$order="";
	$query_rsAdminCallNum = "SELECT * FROM lctr_Oversize_cn ORDER BY LocationDisplayName_cn ASC";
}
...
<html>
<head>
<title>Firestone Locator | Oversize Collection</title>
<?php styles(); ?>
...
<body>
	<?php page_header(); ?>
	<div class="content">
		<h2>Configure oversize collection.</h2>
		<ul>
		...
		<table width="100%" border="0" class="code_table">
			<tr>
				<th colspan="2">&nbsp;</th>
				<th>
						<a href="indexOversize.php?order=left_cn">Left Endpoint </a>
				</th>
				<th>
						<a href="indexOversize.php?order=right_cn">Right Endpoint</a>
				</th>
				<th>
						<a href="indexOversize.php?order=LocationDisplayName_cn">Location
							Display</a>
				</th>
				<th>
						<a href="indexOversize.php?order=BuildingCode_cn">Building</a>
				</th>
				<th>
						<a href="indexOversize.php?order=FloorDB_cn">Floor DB</a>
				</th>
				<th>
						<a href="indexOversize.php?order=LocationMap_cn">Location</a>
				</th>
				<th>
						<a href="indexOversize.php?order=Image_cn">Image</a>
				</th>
				<th>
						<a href="indexOversize.php?order=message_cn">Message</a>
				</th>
				<th>
						<a href="indexOversize.php?order=date_cn">Last Modified</a>
				</th>
			</tr>

			<tr>
				<form action="indexOversize.php" method="post" name="InsertCallNum"
					id="InsertCallNum">
					<th colspan="2"><input name="id_cn" type="hidden"
						value="<?php if (isset($reqRecord['id_cn'])) echo $reqRecord['id_cn']; ?>" /> <?php if (isset($mode)&&$mode="edit") echo "Edit mode"; else echo "Create Mode"; ?>
					</th>
					<td>
						<div align="center" class="input">

							<input name="left" type="text" id="left"
								value="<?php if (isset($reqRecord['left_cn'])) echo $reqRecord['left_cn']; ?>" size="10" />
						</div>
					</td>
					...
					<td>
						<div align="center" class="input">
							<input name="right" type="text" id="right"
								value="<?php if (isset($reqRecord['right_cn'])) echo $reqRecord['right_cn']; ?>" size="10" />
						</div>
					</td>
					...
					<td>
						<div align="center" class="input">
							<input name="location_display" type="text" id="location_display"
								value="<?php if (isset($reqRecord['LocationDisplayName_cn'])) echo $reqRecord['LocationDisplayName_cn']; ?>" size="40" />
						</div>
					</td>
					...
					<td>
						<div align="center" class="input">
							<input name="building" type="text" id="building"
								value="<?php if (isset($reqRecord['BuildingCode_cn'])) echo $reqRecord['BuildingCode_cn']; ?>" size="3" />
						</div>
					</td>
					...
					<td>
						<div align="center" class="input">
							<input name="floordb" type="text" id="floordb"
								value="<?php if (isset($reqRecord['FloorDB_cn'])) echo $reqRecord['FloorDB_cn']; ?>" size="3" />
						</div>
					</td>
					...
					<td>
						<div align="center" class="input">
							<input name="location" type="text" id="location"
								value="<?php if (isset($reqRecord['LocationMap_cn'])) echo $reqRecord['LocationMap_cn']; ?>" size="6" />
						</div>
					</td>
					...
					<td>
						...
						<div align="left" class="input">
								...
								<input name="image" type="text" id="image"
									value="<?php if (isset($reqRecord['Image_cn'])) echo $reqRecord['Image_cn']; ?>" size="20">
						</div>
					</td>


					<td class="input"><input name="message" type="text" id="message"
						value="<?php if (isset($reqRecord['message_cn'])) echo $reqRecord['message_cn']; ?>" size="35"></td>
					<td>
						<div align="center" class="input">
			...
			<?php if (isset($_GET['id_cn']) && $row_rsAdminCallNum['id_cn'] == $_GET['id_cn']) echo "class='edit_row'"?>>
				...
				<td>
						<?php echo $row_rsAdminCallNum['right_cn']; ?>
				</td>
				...
				<td>
						<a class="itemout" href="setEndPoint.php?id=<?php echo $row_rsAdminCallNum['id_cn']; ?>&amp;db=lctr_Oversize_cn" target="_blank"><?php echo $row_rsAdminCallNum['LocationMap_cn']; ?>
						</a>
				</td>
